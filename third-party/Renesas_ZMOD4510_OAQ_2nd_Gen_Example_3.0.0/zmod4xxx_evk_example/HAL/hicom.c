...
hicom_status_t hicom_open(hicom_handle_t *p_handle)
{
    ...
    for (dev_idx = 0; dev_idx < num_hi_speed_devices; dev_idx++) {
        // ask for device name
        ftdi_status = I2C_GetHiSpeedDeviceNameLocIDChannel(
            dev_idx, dev_name, 200, &loc_id, channel, 8, &hi_speed_device_type);
        if (FTC_SUCCESS != ftdi_status) {
            return ftdi_status;
        }

#ifdef HICOM_DEBUG_MESSAGES
        printf("Dbg (HiCom): DevIdx=%lu, DevName=%s, LocID=%lu, "
               "Channel=%s, HiSpeedDeviceType=%lu\n ",
        ...
        if (strcmp(dev_name, HICOM_NAME) == 0) {
            ...
            //  open it
            ftdi_status =
                I2C_OpenHiSpeedDevice(dev_name, loc_id, channel, p_handle);
            if (FTC_SUCCESS != ftdi_status) {
                return ftdi_status;
            }
            ...
            ftdi_status =
                I2C_TurnOffDivideByFiveClockingHiSpeedDevice(*p_handle);
            if (FTC_SUCCESS != ftdi_status) {
                return ftdi_status;
            }

            ftdi_status =
                I2C_SetClock(*p_handle,
                             (DWORD)((60.0e6 / 2 / HICOM_I2C_SPEED) - 1),
                             &clock_freq);
            if (FTC_SUCCESS != ftdi_status) {
                return ftdi_status;
            }
...
hicom_status_t hicom_close(hicom_handle_t handle)
{
    ...
    ftdi_status =
        I2C_SetHiSpeedDeviceGPIOs(handle, TRUE, &low_pins, TRUE, &high_pins);
    if (FTC_SUCCESS != ftdi_status) {
        return ftdi_status;
    }
