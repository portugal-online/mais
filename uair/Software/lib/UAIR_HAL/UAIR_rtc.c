...
/**
  ...
  */
#define UAIR_RTC_DBG_PRINTF(...) LIB_PRINTF(...)
#else
/**
  * @brief not used
  */
#define UAIR_RTC_DBG_PRINTF(...)
#endif /* RTIF_DEBUG */
 ...
 * @return None
 */
static void UAIR_RTC_BkUp_Write_MSBticks(uint32_t MSBticks);
 ...
 * @return MSBticks
 */
static uint32_t UAIR_RTC_BkUp_Read_MSBticks(void);
...
const UTIL_TIMER_Driver_s UTIL_TimerDriver =
{
  ...
  UAIR_RTC_SetTimerContext,
  UAIR_RTC_GetTimerContext,

  UAIR_RTC_GetTimerElapsedTime,
  UAIR_RTC_GetTimerValue,
  UAIR_RTC_GetMinimumTimeout,

  UAIR_RTC_Convert_ms2Tick,
  UAIR_RTC_Convert_Tick2ms,
};

/*System Time driver*/
const UTIL_SYSTIM_Driver_s UTIL_SYSTIMDriver =
{
  UAIR_RTC_BkUp_Write_Seconds,
  UAIR_RTC_BkUp_Read_Seconds,
  UAIR_RTC_BkUp_Write_SubSeconds,
  UAIR_RTC_BkUp_Read_SubSeconds,
  UAIR_RTC_GetTime,
};
...
UTIL_TIMER_Status_t UAIR_RTC_Init(void)
{
  ...
  {
    UAIR_BSP_rtc.IsEnabled.RtcFeatures = UINT32_MAX;

    /*Init RTC*/
    UAIR_BSP_RTC_Init();
    /** Stop Timer */
    UAIR_RTC_StopTimer();
    /** DeActivate the Alarm A enabled by MX during UAIR_BSP_RTC_Init() */
    HAL_RTC_DeactivateAlarm(&UAIR_BSP_rtc, RTC_ALARM_A);
    /*Overload RTC feature enable*/
    UAIR_BSP_rtc.IsEnabled.RtcFeatures = UINT32_MAX;

    /*Enable Direct Read of the calendar registers (not through Shadow) */
    HAL_RTCEx_EnableBypassShadow(&UAIR_BSP_rtc);
    /*initialise MSB ticks*/
    UAIR_RTC_BkUp_Write_MSBticks(0);
...
UTIL_TIMER_Status_t UAIR_RTC_StartTimer(uint32_t timeout)
{
  ...
  UAIR_RTC_DBG_PRINTF("Start timer: time=%d, alarm=%d\n\r",  GetTimerTicks(), timeout);
  /* starts timer*/
...
UTIL_TIMER_Status_t UAIR_RTC_StopTimer(void)
{
  ...
  __HAL_RTC_ALARM_CLEAR_FLAG(&UAIR_BSP_rtc, RTC_FLAG_ALRAF);
...
uint32_t UAIR_RTC_SetTimerContext(void)
{
  ...
  UAIR_RTC_DBG_PRINTF("UAIR_RTC_SetTimerContext=%d\n\r", RtcTimerContext);
  /*return time context*/
...
{
  UAIR_RTC_DBG_PRINTF("UAIR_RTC_GetTimerContext=%d\n\r", RtcTimerContext);
  /*return time context*/
  return RtcTimerContext;
}
...
{
  uint32_t delayTicks = UAIR_RTC_Convert_ms2Tick(delay);
...
void HAL_RTCEx_SSRUEventCallback(RTC_HandleTypeDef *hrtc)
{
  ...
  UAIR_RTC_DBG_PRINTF(">>Handler SSRUnderflow at %d\n\r", GetTimerTicks());
  /*Increment MSBticks*/
  uint32_t MSB_ticks = UAIR_RTC_BkUp_Read_MSBticks();
  UAIR_RTC_BkUp_Write_MSBticks(MSB_ticks + 1);
}
...
/**
  ...
  * @return time seconds
  */
uint32_t UAIR_RTC_GetTime(uint16_t *mSeconds)
{
  uint64_t ticks;
  uint32_t timerValueLsb = GetTimerTicks();
  uint32_t timerValueMSB = UAIR_RTC_BkUp_Read_MSBticks();
  ...
  *mSeconds = UAIR_RTC_Convert_Tick2ms(ticks);
...
{
  HAL_RTCEx_BKUPWrite(&UAIR_BSP_rtc, RTC_BKP_SECONDS, Seconds);
}
...
{
  HAL_RTCEx_BKUPWrite(&UAIR_BSP_rtc, RTC_BKP_SUBSECONDS, SubSeconds);
}
...
{
  return HAL_RTCEx_BKUPRead(&UAIR_BSP_rtc, RTC_BKP_SECONDS);
}
...
{
  return HAL_RTCEx_BKUPRead(&UAIR_BSP_rtc, RTC_BKP_SUBSECONDS);
}

static void UAIR_RTC_BkUp_Write_MSBticks(uint32_t MSBticks)
{
  HAL_RTCEx_BKUPWrite(&UAIR_BSP_rtc, RTC_BKP_MSBTICKS, MSBticks);
}

static uint32_t UAIR_RTC_BkUp_Read_MSBticks(void)
{
  uint32_t MSBticks;
  MSBticks = HAL_RTCEx_BKUPRead(&UAIR_BSP_rtc, RTC_BKP_MSBTICKS);
  return MSBticks;
}
